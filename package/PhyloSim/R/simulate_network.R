...
#' @param NC A function returning the number of descendents (children) for each
#' new node created (default: a function returning the value 3).
#' @param NP A function returning the number of ascendants (parents) for each
...
#' time per generation).
#' @param maxDst The maximum distance between the first parent drawn and any
...
#' 
#' @export
simulate_network <- function(I, NS, NN, traitMod, initState, initValue,
                             NC = function() 3L, NP = function() 2L,
                             gamma.shape = 5, gamma.scale = 5e-4, timestep = 1,
                             maxDst = 4*timestep, prob = c(0.3,rep(0.175,4L)),
                             removeGapOnly = TRUE, verbose = TRUE) {
